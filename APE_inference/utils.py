...
def cal_criterion(cfg, clip_weights, cache_keys, only_use_txt=False):
    ...
    save_file = '{}/criterion_{}_{}_shot.pt'.format(save_path, cfg['backbone'].replace('/', ''), cfg['shots'])
...
class APE_Training(nn.Module):
    def __init__(self, cfg, clip_weights, clip_model, cache_keys):
        ...
        self.original_indices = self.indices.clone()  # Store the original indices
        self.original_features = cache_keys.reshape(self.cate_num, self.shots, self.feat_dim).clone()  # Store the original features
        self.refined_indices = None  # To store the refined indices after forward pass
        self.refined_features = None  # To store the refined features after forward pass
    ...
    def forward(self, cache_keys, clip_weights, cache_values):
        ...
        # Store the refined indices and features for visualization
        self.refined_indices = self.indices
        self.refined_features = new_cache_keys.half()
