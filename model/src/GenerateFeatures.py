...
class GenerateFeatures:
    ...
    def generate_features(self):
        print(f'Current category is {self.cls_name}')
        train_path = f'./data/cosmetic/images/train/{self.cls_name}'
        
        # base_dir 내의 하위 폴더 목록 가져오기
        subfolders = next(os.walk(train_path))[1]

        # 첫 번째 하위 폴더 경로
        if subfolders:
            first_subfolder = os.path.join(train_path, subfolders[0])
        else:
            print("하위 폴더가 없습니다.")
            first_subfolder = None

        # 첫 번째 하위 폴더에서 .jpg로 끝나는 파일 개수 계산
        jpg_count = 0

        if first_subfolder:
            for root, dirs, files in os.walk(first_subfolder):
